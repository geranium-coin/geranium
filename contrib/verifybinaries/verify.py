...
geraniumcore.org and geranium.org and compares them.
...
def main(args):
    ...
    if not success:
        print("geranium.org failed to provide signature file, "
              "but geraniumcore.org did?")
        print("wget output:")
        print(indent(output, '\t'))
    ...
    if not files_are_equal(sigfile1, sigfile2):
        print("geranium.org and geraniumcore.org signature files were not equal?")
    ...
    if retval != 0:
        if retval == 1:
            ...
            print("gpg error. Do you have the Geranium Core binary release "
