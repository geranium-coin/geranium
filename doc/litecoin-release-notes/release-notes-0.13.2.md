Geranium Core version 0.13.2 is now available from:

  <https://download.geranium.org/geranium-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a geranium
...
ECDSA signatures inside Geranium transactions now use validation using
...
Previous versions of Geranium Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Geranium Core was to raise the default minimum
relay fee.

Geranium Core 0.13.2 will have a strict maximum size on the mempool. The
...
Geranium Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running geraniumd without having to do any manual configuration.
...
Geranium Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Geranium Core will relay transactions with
...
entirely in Geranium Core 0.13, and it is at this time undecided whether the
...
Geranium Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Geranium Core automatically creates a hidden service to listen on, without
manual configuration. Geranium Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Geranium Core is listening, and
...
Geraniumd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Geranium
Core to use floating fees. By default, Geranium Core will use floating
fees.
...
(default: `0.0002` GEAM/kB).

At all times, Geranium Core will cap fees at `-maxtxfee=<x>` (default:
0.10) GEAM.
Furthermore, Geranium Core will never create transactions paying less than
...
`geranium.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 27555 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 27555 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /geraniumrpc>
        ProxyPass http://127.0.0.1:7555/
        ProxyPassReverse http://127.0.0.1:7555/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass geraniumd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< geraniumrpc:<password>
    </Location>
...
Restarting geraniumd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in geranium.conf
...
geranium-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/geranium-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Geranium Core code base has
...
- `geranium-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `geranium-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Geranium scripting system that in combination with
...
Geranium transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Geranium Core.
...
The two phases are distinct in the Geranium-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Geranium wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Geranium.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Geranium block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of geraniums they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of geraniums being spent is exactly
...
- **Increased security for multisig:** Geranium addresses (both P2PKH addresses
  ...
  believed to be provided by Geranium's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Geranium's current ledger (called the UTXO set),
  ...
  note that Geranium Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Geranium users to individually opt-in to almost any change in the Geranium
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Geranium Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Geranium
client to Geranium Core 0.13.2 to signal for segwit support. When a
...
Since Geranium Core 0.10.0, nodes have defaulted to only relaying and
...
- `geranium-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `geranium-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
