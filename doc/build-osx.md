...
This guide describes how to build geraniumd, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Geranium Core from source.
...
### 4. Clone Geranium repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Geranium Core repository to a directory.
...
It is not necessary to build wallet functionality to run `geraniumd` or  `geranium-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/geranium/geranium/blob/master/doc/descriptors.md), `sqlite` is required.
...
Geranium Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/geranium/geranium/issues/7714).
...
You can deploy a `.dmg` containing the Geranium Core application using `make deploy`.
...
## Building Geranium Core

### 1. Configuration

There are many ways to configure Geranium Core, here are a few common examples:
...
Run the following in your terminal to compile Geranium Core:
...
## Running Geranium Core

Geranium Core should now be available at `./src/geraniumd`.
If you compiled support for the GUI, it should be available at `./src/qt/geranium-qt`.

The first time you run `geraniumd` or `geranium-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Geranium/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Geranium"

touch "/Users/${USER}/Library/Application Support/Geranium/geranium.conf"
...
./src/qt/geranium-qt -server # Starts the geranium-qt server mode, allows geranium-cli control
```
