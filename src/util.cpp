...
const char * const GERANIUM_CONF_FILENAME = "geranium.conf";
const char * const GERANIUM_PID_FILENAME = "geraniumd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "geraniumd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Geranium
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Geranium
    // Mac: ~/Library/Application Support/Geranium
    // Unix: ~/.geranium
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Geranium";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Geranium";
#else
    // Unix
    return pathRet / ".geranium";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", GERANIUM_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Geranium Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Geranium Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Geranium Core developers";
    }
