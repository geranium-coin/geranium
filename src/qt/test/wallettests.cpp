...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<GeraniumAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_geranium-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_geranium-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_geranium-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = GeraniumUnits::formatWithUnit(unit, balance, false, GeraniumUnits::separatorAlways);
    ...
    // Amount input
    GeraniumAmountField* amountInput = receiveCoinsDialog.findChild<GeraniumAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(1).indexOf(QString("URI: geranium:")) != -1);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_geranium-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
