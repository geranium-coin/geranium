...
QString formatGeraniumURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("geranium:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(GeraniumUnits::format(GeraniumUnits::GEAM, info.amount, false, GeraniumUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", GERANIUM_CONF_FILENAME));
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("geranium-%s.desktop", chain);
}
