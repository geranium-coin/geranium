...
const std::string GeraniumGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
GeraniumGUI::GeraniumGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
...
void GeraniumGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Geranium address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and geranium: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &GeraniumGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(sendCoinsMenuAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsMenuAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &GeraniumGUI::gotoReceiveCoinsPage);
    connect(receiveCoinsMenuAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsMenuAction, &QAction::triggered, this, &GeraniumGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &GeraniumGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    connect(optionsAction, &QAction::triggered, this, &GeraniumGUI::optionsClicked);
    connect(toggleHideAction, &QAction::triggered, this, &GeraniumGUI::toggleHidden);
    connect(showHelpMessageAction, &QAction::triggered, this, &GeraniumGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &GeraniumGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &GeraniumGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &GeraniumGUI::setCurrentWallet);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &GeraniumGUI::setCurrentWallet);
            connect(activity, &CreateWalletActivity::finished, activity, &QObject::deleteLater);
            activity->create();
        });
    ...
    connect(new QShortcut(QKeySequence(Qt::CTRL + Qt::SHIFT + Qt::Key_C), this), &QShortcut::activated, this, &GeraniumGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL + Qt::SHIFT + Qt::Key_D), this), &QShortcut::activated, this, &GeraniumGUI::showDebugWindow);
}

void GeraniumGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void GeraniumGUI::createToolBars()
{
    if(walletFrame)
    {
...
#ifdef ENABLE_WALLET
void GeraniumGUI::setWalletController(WalletController* wallet_controller)
{
...
void GeraniumGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame) return;
...
void GeraniumGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void GeraniumGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void GeraniumGUI::setWalletActionsEnabled(bool enabled)
{
...
void GeraniumGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
    ...
    connect(trayIcon, &QSystemTrayIcon::activated, this, &GeraniumGUI::trayIconActivated);
#else
    // Note: On macOS, the Dock icon is used to provide the tray's functionality.
    MacDockIconHandler *dockIconHandler = MacDockIconHandler::instance();
    connect(dockIconHandler, &MacDockIconHandler::dockIconClicked, this, &GeraniumGUI::macosDockIconActivated);
...
#ifndef Q_OS_MAC
void GeraniumGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
#else
void GeraniumGUI::macosDockIconActivated()
{
    show();
    activateWindow();
}
...
void GeraniumGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void GeraniumGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void GeraniumGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void GeraniumGUI::showDebugWindowActivateConsole()
{
...
void GeraniumGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void GeraniumGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void GeraniumGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void GeraniumGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void GeraniumGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void GeraniumGUI::gotoVerifyMessageTab(QString addr)
{
...
void GeraniumGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &GeraniumGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &GeraniumGUI::show);
                e->ignore();
            }
        }
    }
...
void GeraniumGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void GeraniumGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void GeraniumGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
...
void GeraniumGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void GeraniumGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool GeraniumGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool GeraniumGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void GeraniumGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void GeraniumGUI::updateWalletStatus()
{
    if (!walletFrame) {
        return;
    }
...
void GeraniumGUI::updateProxyIcon()
{
    std::string ip_port;
...
void GeraniumGUI::updateWindowTitle()
{
...
void GeraniumGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void GeraniumGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void GeraniumGUI::detectShutdown()
{
...
void GeraniumGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void GeraniumGUI::subscribeToCoreSignals()
{
...
void GeraniumGUI::unsubscribeFromCoreSignals()
{
      ...
      m_platform_style{platformStyle}
{
    ...
    QList<GeraniumUnits::Unit> units = GeraniumUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const GeraniumUnits::Unit unit : units)
    {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, GeraniumUnits::longName(unit)));
    }
...
{
    setText(GeraniumUnits::longName(newUnits));
}
