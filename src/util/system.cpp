...
// For details see https://github.com/geranium/geranium/pull/22348.
...
const char * const GERANIUM_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/geranium/geranium/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //geranium-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", GERANIUM_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Geranium
    // macOS: ~/Library/Application Support/Geranium
    // Unix-like: ~/.geranium
#ifdef WIN32
    // Windows
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Geranium Core copyright is not removed by accident
    if (copyright_devs.find("Geranium Core") == std::string::npos) {
