// Copyright (c) 2018 The Geranium Core developers
...
// geranium includes.
#include <..\src\script\geraniumconsensus.h>
...
int main()
{
    std::cout << "geraniumconsensus version: " << geraniumconsensus_version() << std::endl;
    ...
    auto op0Result = geraniumconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, geraniumconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
